...
public class ProfissaoPersonagemRepository {
    ...
    public ProfissaoPersonagem retornaProfissaoModificada(
        ...
        for (ProfissaoPersonagem profissaoPersonagem : profissoes) {
            if (profissaoPersonagem.getNome().equals(trabalho.getProfissao())) {
                return profissaoPersonagem;
            }
        }
        return null;
    }
    ...
    public LiveData<Resource<ArrayList<ProfissaoPersonagem>>> recuperaProfissoesPersonagem() {
        MutableLiveData<Resource<ArrayList<ProfissaoPersonagem>>> profissoesRecuperadas = new MutableLiveData<>();
        ouvinteProfissoesPersonagem = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot dn : dataSnapshot.getChildren()) {
                    ProfissaoPersonagem profissaoPersonagem = dn.getValue(ProfissaoPersonagem.class);
                ...
                List<ProfissaoPersonagem> profissoesServidor = Collections.synchronizedList(new ArrayList<>());
                ArrayList<Task<DataSnapshot>> tarefas = new ArrayList<>(profissoes.size());
                for (ProfissaoPersonagem profissaoPersonagem : profissoes) {
                    tarefas.add(referenciaListaProfissoes.child(profissaoPersonagem.getId()).get());
                }
                Tasks.whenAllSuccess(tarefas).addOnCompleteListener(backGroundExecutor, tarefasCombinadas -> {
                    if (tarefasCombinadas.isSuccessful()) {
                        for (int i = 0; i < tarefasCombinadas.getResult().size(); i++) {
                            ...
                            ProfissaoPersonagem profissaoPersonagem = ds.getValue(ProfissaoPersonagem.class);
                            if (profissaoPersonagem == null) continue;
                            ProfissaoPersonagem profissaoPersonagem1 = profissoes.get(i);
                            profissaoPersonagem1.setNome(profissaoPersonagem.getNome());
                            profissoesServidor.add(profissaoPersonagem1);
                        }
                        profissoesServidor.sort(Comparator.comparing(ProfissaoPersonagem::getExperiencia).reversed());
        ...
        ouvinteListaProfissoes = new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot dn : snapshot.getChildren()) {
                    ProfissaoPersonagem valor = dn.getValue(ProfissaoPersonagem.class);
                    if (valor == null) continue;
                    ProfissaoPersonagem profissaoPersonagem = new ProfissaoPersonagem();
                    profissaoPersonagem.setId(valor.getId());
                    profissaoPersonagem.setExperiencia(0);
                    profissaoPersonagem.setPrioridade(false);
                    tarefas.add(referenciaProfissoesPersonagem.child(profissaoPersonagem.getId()).setValue(profissaoPersonagem));
                }
    ...
    public LiveData<Resource<Void>> modificaProfissaoPersonagem(ProfissaoPersonagem profissaoPersonagemModificada) {
        MutableLiveData<Resource<Void>> liveData = new MutableLiveData<>();
        ProfissaoPersonagem profissaoPersonagem = new ProfissaoPersonagem();
        profissaoPersonagem.setId(profissaoPersonagemModificada.getId());
        profissaoPersonagem.setExperiencia(profissaoPersonagemModificada.getExperiencia());
        profissaoPersonagem.setPrioridade(profissaoPersonagemModificada.isPrioridade());
        referenciaProfissoesPersonagem.child(profissaoPersonagemModificada.getId()).setValue(profissaoPersonagem)
